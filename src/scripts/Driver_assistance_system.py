...
def build_compact_display(
    ...
    advisory_text = format_advisory_speed(speed_advisory_kph)
    parts.append(f"ADVSPEED {advisory_text}")

    parts.append(f"{remaining_distance_km:.1f} km")
...
def format_speed_display(segment):
    ...
    if source == "speed_osm" and speed is not None:
...
def format_advisory_speed(speed_advisory_kph):
    if speed_advisory_kph is None:
        return "--"
    return f"{speed_advisory_kph:.0f} kph"
...
def get_upcoming_intersection_warning(current_distance_m, intersection_details, lookahead_m=150.0):
    ...
    if nearest.get("is_signalized"):
        return f"Signalized intersection ahead in {distance_ahead:.1f} m"
    elif nearest.get("is_stop_controlled"):
        return f"Stop-controlled intersection ahead in {distance_ahead:.1f} m"
    else:
        return f"Intersection ahead in {distance_ahead:.1f} m"


def get_control_on_current_segment(current_distance_m, segment_length_m, intersection_details, window_m=40.0):
    segment_start = current_distance_m
    segment_end = current_distance_m + segment_length_m
    ...
    for intersection in intersection_details:
        ...
        if (segment_start - window_m) <= intersection_distance <= (segment_end + window_m):
...
def compute_remaining_distance_km(total_length_km, current_distance_m):
    return max(total_length_km - current_distance_m / 1000.0, 0.0)


def compute_remaining_time_min(remaining_distance_km, average_speed_kph):
    if average_speed_kph is None or average_speed_kph <= 0:
        return None
    return (remaining_distance_km / average_speed_kph) * 60.0
...
with open("output/FM_908_to_CR_316.json", "r") as f:
...
def make_status_card(parent, title, wraplength=260, value_font=("Arial", 18, "bold")):
...
speed_card, speed_box = make_status_card(status_frame, "SPEED", wraplength=180, value_font=("Arial", 20, "bold"))
speed_card.grid(row=0, column=0, sticky="nsew", padx=8, pady=6)

curve_card, curve_box = make_status_card(status_frame, "CURVE", wraplength=220, value_font=("Arial", 20, "bold"))
curve_card.grid(row=0, column=1, sticky="nsew", padx=8, pady=6)

control_card, control_box = make_status_card(status_frame, "ROAD WAY ALERT", wraplength=360, value_font=("Arial", 17, "bold"))
control_card.grid(row=0, column=2, sticky="nsew", padx=8, pady=6)

distance_card, distance_box = make_status_card(status_frame, "REMAIN", wraplength=180, value_font=("Arial", 20, "bold"))
distance_card.grid(row=0, column=3, sticky="nsew", padx=8, pady=6)
...
def update_driver_alert_gui(output_row, driver_alert_message):
    ...
    distance_box.config(
        text=f"{output_row['remaining_distance_km']:.1f} km",
        bg="white",
        fg="#111827"
    )
    ...
    segment_text = (
        ...
        f"Remaining Distance   : {output_row['remaining_distance_km']:.2f} km\n"
...
average_speed_kph = edge_attribute_summary.get("weighted_avg_speed_limit_kph")
total_length_km = route_metrics["length_km"]

for segment in edge_attributes:
    segment_length_m = segment.get("segment_length_m", 0.0)
    ...
    remaining_distance_km = compute_remaining_distance_km(total_length_km, current_distance_m)
    remaining_time_min = compute_remaining_time_min(remaining_distance_km, average_speed_kph)
    ...
    current_distance_m += segment_length_m
